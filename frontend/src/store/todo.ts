...
export interface TodosFetchedStarted {
  type: "TODOS_FETCHED_STARTED";
}
...
export interface TodoCreatedStarted {
  type: "TODO_CREATED_STARTED";
}
...
export interface TodoCompletedStarted {
  type: "TODO_COMPLETED_STARTED";
}
...
export const fetchTodos = () => async (dispatch: Dispatch) => {
  ...
  if (!!response && !!response.todos) {
    ...
    dispatch({
      type: "TODO_ERRORED",
      payload: { fetchError: response?.error ?? "Error fetching Todo items" },
    });
  }
};
...
export const createTodo = (todo: TodoItem) => async (dispatch: Dispatch) => {
  ...
  if (!!response && !!response.todo) {
    ...
    dispatch({
      type: "TODO_ERRORED",
      payload: { createError: response?.error ?? "Error creating Todo item" },
    });
  }
};
...
export const completeTodo = (guid: string) => async (dispatch: Dispatch) => {
  ...
  if (!!response && !!response?.todo) {
    ...
    dispatch({
      type: "TODO_ERRORED",
      payload: {
        completeError: response?.error ?? "Error completing Todo item",
      },
    });
  }
};
...
export const selectTodos = (state: RootState): TodoItem[] =>
  state.todo.todos ?? [];
export const selectCompleteTodos = (state: RootState): TodoItem[] =>
  state.todo.todos.filter((item) => item.complete) ?? [];
export const selectIncompleteTodos = (state: RootState): TodoItem[] =>
  state.todo.todos.filter((item) => !item.complete) ?? [];
